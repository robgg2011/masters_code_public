...
def main():
    ...
    weights, optim_state = init_model(subkey)
    ...
    axs[2].plot(batch_accuracy)
    axs[2].set_title("Batch training accuracy")
    ...
    if config.create_umap:
        ...
        for x, y in test:
            ...
            key, subkey = random.split(key)
            _, _, out, goodness, latent_act = ff_process(x, jnp.zeros_like(y), best_params[0], optim_state, subkey, plasticity=False, record_latent=True)
        ...
        plt.title('UMAP projection of latent representation (target labels)', fontsize=24)
        plt.show()
        ...
        plt.gca().set_aspect('equal', 'datalim')
        plt.colorbar(boundaries=np.arange(11)-0.5).set_ticks(np.arange(10))
        plt.title('UMAP projection of latent representation (output labels)', fontsize=24)
        plt.show()
